# Changelog — Multisite Status Server (Acuity)
...
Initial build. Server half of the Multisite Status (Acuity) client/server pair.

### Added

- **Schema** (`hook_schema`): `acuity_multisite_status_site`,
  `acuity_multisite_status_project`, `acuity_multisite_status_site_project`.
...
- **Permission**: dedicated `administer acuity multisite status`.
...
- **API — `POST api/v1/acuity-multisite-status/report`**: Bearer-key auth
...
- **API — `GET api/v1/acuity-multisite-status/status`**: authenticated health /
...
- **Settings page** (`admin/config/acuity-multisite-status-server/settings`, MENU_LOCAL_TASK tab):
  ...
  - Default config in `config/acuity_multisite_status_server.settings.json`.
- **Project security review form** (`admin/config/acuity-multisite-status-server/projects/{id}/review`):
...
- **Authoritative latest-version check** (`acuity_multisite_status_server.fetch.inc`):
...
- **Version comparator** (`acuity_multisite_status_server_compare_versions`): strips
...
- **Project type filter** in Views: custom `views_handler_filter_in_operator` subclass
  (`acuity_multisite_status_server_handler_filter_project_type`) provides a
...
- **site_project join definitions**: `table.join` entries added to
  `acuity_multisite_status_site_project` so Views can traverse the bridge table in
